...
for question in tutorial_questions:
    try:
        ...
        # Check if the error message is a dictionary
        if hasattr(e, 'args') and len(e.args) > 0:
            error_info = e.args[0]  # Get the first argument of the exception
            if isinstance(error_info, dict):  # Check if it's a dictionary
                # Print the 'message' field
                print("Error message:", error_info.get('message', 'No message found'))
                print("Model output:", error_info.get('model_output', 'No model output found'))
            else:
                print("Error information is not a dictionary:", error_info)
        else:
